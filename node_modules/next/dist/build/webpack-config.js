...
async function getBaseWebpackConfig(dir, { buildId , config , compilerType , dev =false , entrypoints , isDevFallback =false , pagesDir , reactProductionProfiling =false , rewrites , runWebpackSpan , target =_constants1.COMPILER_NAMES.server , appDir , middlewareMatchers , noMangling =false  }) {
    ...
    async function handleExternals(context, request, dependencyType, layer, getResolve) {
        ...
        const isLocalCallback = (localRes)=>{
            ...
            layer === _constants.WEBPACK_LAYERS.server ? false : /next[/\\]dist[/\\](esm[\\/])?(shared|server)[/\\](?!lib[/\\](router[/\\]router|dynamic))/.test(localRes);
            if (isNextExternal) {
    ...
    const cache = {
        ...
        version: `${"13.0.3-canary.4"}|${configVars}`,
